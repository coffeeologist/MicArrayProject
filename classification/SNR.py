...
def loadAudio(audioPath, size, absStart, absEnd):
    command = "ffmpeg -i "+ audioPath +" -af \"highpass=f=195, lowpass=f=3843\" filtered.wav"
    ...
    result = librosa.stream("filtered.wav",
                            offset=absStart,
...
def calculateValues(stream, size, hopLength, sampleRate, absStart):
    ...
    for index, timeBlock in enumerate(stream):
        ...
        shortTermPower.append(np.sum(np.square(spectrogramMagnitude)))
    ...
    return shortTermPower, timeIntervals
...
def createDataFrame(shortTermPower, timeIntervals):
    start = [i[0] for i in timeIntervals]
    end = [i[1] for i in timeIntervals]
    
    tmp = np.array([start, end, shortTermPower])
    tmp = np.transpose(tmp)

    return pd.DataFrame(tmp, columns=["Start", "End", "Short Term Power"])
...
shortTermPower, timeIntervals = calculateValues(audio, int(sys.argv[2]), hopLength, sampleRate, absStart)
values = createDataFrame(shortTermPower, timeIntervals)
