...
def classify_with_purity(labels, winStart, winEnd):
    d = {}
    for indexSol, rowSol in labels.iterrows():
        labStart = float(rowSol['to'])
        labEnd = float(rowSol['from'])
        ...
        # Need to account for times being deleted in convolution
        if(labStart <= winStart and winEnd <= labEnd):
            print(lab[0:1])
            d[(labStart, labEnd)] = lab[0:1]

    # TODO: Configure which labels can be used
    # Currently, only works for A, D, S
    aTimes = dict(filter(lambda label: label == "A", d.values()))
    dTimes = dict(filter(lambda label: label == "D", d.values()))
    sTimes = dict(filter(lambda label: label == "S", d.values()))

    if aTimes:
        totalATime = reduce(operator.add, list(map(lambda start,end: end-start, aTimes.keys())))
    else:
        totalATime = 0.0
    if dTimes:
        totalDTime = reduce(operator.add, list(map(lambda start,end: end-start, dTimes.keys())))
    else:
        totalDTime = 0.0
    if sTimes:
        totalSTime = reduce(operator.add, list(map(lambda start,end: end-start, sTimes.keys())))
    else:
        totalSTime = 0.0

    totalTime = totalATime + totalDTime + totalSTime
    
    finalLabel = ""
    finalTime = 0.0
    if totalATime > totalDTime:
        if totalATime > totalSTime:
            finalLabel = "A"
            finalTime = totalATime
        else:
            finalLabel = "S"
            finalTime = totalSTime
    elif totalDTime > totalSTime:
        finalLabel = "D"
        finalTime = totalDTime
    else:
        finalLabel = "S"
        finalTime = totalSTime
    
    # TODO: Configure threshold
    if (finalTime/totalTime) >= 0.9:
        return finalLabel
    return "DUMPED"
...
def findValForEachSeg(labels, path, metric, intervalLen, destCSV):
    ...
    for index, row in df.iterrows():
        ...
        labelClassification.append(classify_with_purity(labels, winStart, winEnd))
    ...
    for index, row in df2.iterrows():
        ...
        labelClassification2.append(classify_with_purity(labels, winStart, winEnd))
